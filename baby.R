...
# expand.grid combines vectors to make a data frame. Making a df of decades
 decadeState <- expand.grid(Decade = seq(from = 1910, to = 2010, by = 10), State = stateAbbr)

# Adding "s" at the end of decades
decadeState$Decade = paste0(decadeState$Decade,"s")

# converting years to decades and adding "s"
...
# pipe object forward into function using %>%
